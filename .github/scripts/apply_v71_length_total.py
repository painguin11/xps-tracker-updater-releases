...
# A cleaning length can have OCR output but still have zero valid candidates
# (for example 114 read as 6114/36114). Force the existing border-free
# consensus pass in that case instead of leaving it blank.
replace_once(
    ...
    'cleaning consensus trigger')


# Independent printed-total reading and pure reconciliation helpers.
marker="def _table_row_bands(img, min_y_ratio=.10, max_y_ratio=.86):\n"
helpers=r'''def _choose_printed_total(cands):
...
if marker not in text:
    raise SystemExit('Table-row marker not found')
text=text.replace(marker,helpers+marker,1)


# Save the independently read footer total on the prepared layout.
replace_once(
    """        seen.add(key); rows.append(rec)\n        if on_row: on_row(rec)\n    return rows\n\n\ndef parse_year15_manholes""",
    """        seen.add(key); rows.append(rec)\n        if on_row: on_row(rec)\n    prepared['printed_total_info']=_read_pair_table_printed_total(\n        img,bands,table,val_box,up_box,dn_box,date_box)\n    return rows\n\n\ndef parse_year15_manholes""",
    'pair parser total metadata')


# Persistent analysis-state bookkeeping.
replace_once(
...
replace_once(
    ...
    'clear total validations')


# Give unresolved total failures the strongest summary styling.
replace_once(
...
replace_once(
    ...
    'summary total warning priority')


# GUI reconciliation and user-verification methods.
methods=r'''    def _total_check_records(self,check):
...
'''
marker="    def edit_selected(self):\n"
if marker not in text:
    raise SystemExit('edit_selected marker not found')
text=text.replace(marker,methods+marker,1)


# Start each analysis with a fresh total-source map.
replace_once(
...
replace_once(
    ...
    'analysis total source map')


# Capture every pair page's independent total metadata, then reconcile after all
# pages for the work order have populated the live summary.
replace_once(
...
replace_once(
    ...
    'run work-order total reconciliation')


# Row edits immediately recalculate the work-order total check.
replace_once(
    ...
    'edit recalculates totals')


# Block spreadsheet writes until every available printed-total check reconciles.
replace_once(
    ...
    'block unresolved total writes')


# Make the overall status call out unresolved total failures separately.
replace_once(
