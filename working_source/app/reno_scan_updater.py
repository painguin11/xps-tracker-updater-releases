...
# Asset-ID syntax is inferred from the selected master workbook. The inferred
...
def _load_master_index_once(path):
    ...
    try:
        ...
        if year15 or phase2:
            ...
            pipes={}; pipe_by_id={}; asset_format_samples=[]
...
def orient_and_classify(page):
    ...
    # Work-order form: the number is machine-typed in pink/magenta. Use that
...
def ocr_workorder_guesses(page, master_index=None, expect_manhole_count=False):
    ...
    universal_wo_crops=[
        ...
        crop(.11,.070,.31,.105),  # Reno tight W/O value
        crop(.11,.012,.31,.052),  # upper W/O value position
    ]
...
def _ocr_asset_candidates(cell_img, fast_plain=False, asset_format=None):
    ...
    for vi,im in enumerate(variants):
        for psm in ((7,) if fast_plain else ((7,6) if vi<2 else (7,))):
            ...
            if txt:
                rule=asset_format if isinstance(asset_format,dict) else None
                if rule:
                    formatted=_printed_asset_tokens(txt,asset_format)
...
def _batch_pair_endpoint_candidates(img,bands,table,box,asset_format=None):
...
def _batch_pair_endpoint_full_candidates(img,bands,table,box,asset_format=None,known_items=None):
...
def _ocr_known_r2_candidates(cell_img, known_items, asset_format=None):
    ...
    for image in variants:
        for psm in (7,6):
            ...
            if _asset_format_requires_dash(asset_format) and '-' not in text:
                continue
            key=asset_key(text)
            if key in known_r2:
...
def _confirmed_suffix_asset_candidates(cell_img,known_items,asset_format=None):
    ...
    for view in views:
        seen={asset_key(value) for value in _new_suffix_asset_candidates(
            _ocr_asset_candidates(view,fast_plain=True,asset_format=asset_format),known_items)}
        for key in seen:
...
def _keep_unresolved_pair_row(up_value,down_value,length_value,row_date,asset_format=None):
    ...
    satisfy that format before an unresolved row may survive. This prevents
    ...
    """
    if isinstance(asset_format,dict) and asset_format.get('mode')!='generic':
...
def _choose_length(cands, expected=None):
    cands=[float(x) for x in cands if _valid_row_length_value(x)]
...
def _choose_cleaning_length(cands, expected=None):
    ...
    """
    values=[float(x) for x in cands if _valid_row_length_value(x)]
...
def _direct_pair_length_candidates(cell_img):
...
def _stable_numeric_vote(cands,min_votes=2):
    values=[float(x) for x in (cands or []) if _valid_row_length_value(x)]
...
def parse_pipe_list(page, master_index, quick_text, on_row=None, on_progress=None):
    ...
    for y1,y2 in bands:
        ...
        full_pid_candidates=_ocr_asset_candidates(psr_img,fast_plain=False,asset_format=asset_format)
...
def parse_manhole_list(page, master_index, quick_text, on_row=None, on_progress=None):
    ...
    for i in range(40):
        ...
        full_sid_candidates=_ocr_asset_candidates(id_img,fast_plain=False,asset_format=asset_format)
...
def _master_assisted_endpoint_columns(img,bands,table,column_boxes,master_index):
    ...
    for ci,box in enumerate(column_boxes):
        for ri,(y1,y2) in enumerate(sample_bands):
            ...
            observed[ci][ri]=_ocr_asset_candidates(cell,fast_plain=True,asset_format=asset_format)
    scored=[]
    ...
    for band_index,(y1,y2) in enumerate(bands):
        ...
        def read_id(box,fast=True):
            ...
            if y2-y1>typical_band*1.45:
                ...
                obs+=_ocr_asset_candidates(cell[int(ch*.38):,:],fast_plain=fast,asset_format=asset_format)
        ...
        if not match and match_status!='NEW PIPE' and not authoritative_pair:
            ...
            up_obs=list(dict.fromkeys(up_obs+_ocr_known_r2_candidates(cut(up_box),endpoint_items,asset_format=asset_format)))
        ...
        if kind=='pipes' and not match and match_status=='NEW PIPE':
            ...
            up_confirmed=_confirmed_suffix_asset_candidates(cut(up_box),endpoint_items,asset_format=asset_format)
        ...
        if kind=='cleaning':
            ...
            if value_candidates:
                ...
                value=_choose_cleaning_length(value_candidates,None)
                distinct={float(x) for x in value_candidates if _valid_row_length_value(x)}
        ...
        up_preview=cut(up_box); dn_preview=cut(dn_box)
        rec['_field_previews']={
...
def pipe_group_physical_count(records):
...
class LayoutConfirmDialog(tk.Toplevel):
    ...
    def __init__(self,parent,layout,page_number):
        ...
        labels={'up':'Upstream node (UP_MH)','down':'Downstream node (DN_MH)',
                'value':'Wheel Walk' if layout['kind']=='cleaning' else 'Surveyed/Video Length','date':'Date'}
        start=3
    ...
    def __init__(self, parent, guesses):
        ...
        if guesses.get('expect_manhole_count'):
            ...
            if crop is not None and getattr(crop,'size',0):
                ...
                ttk.Label(self,text='Description preview unavailable — check PDF',foreground='#8A5200').grid(row=next_row,column=2,sticky='w',padx=(6,12),pady=4)
            next_row+=1
        ...
        next_row+=1
        ttk.Label(self,text='Check each scan image beside its field, pre-filled text is only a suggestion.',foreground='#8A5200').grid(row=next_row,column=0,columnspan=3,padx=12,pady=(5,8))
        next_row+=1
...
class App(tk.Tk):
    ...
    def _msa_pair_indices_for_record(self,index):
        try: index=int(index)
    ...
    def commit_extracted_record(self,rec,current_wo,use_date,idx,page_number,processed):
        ...
        for key in ('upstream','downstream','asset','activity_value','date'):
    ...
    def prompt_total_check(self,check):
        ...
        self._retry_length_total_mismatch(check,all_rows=False,force=True)
        self.refresh_total_check(check)
